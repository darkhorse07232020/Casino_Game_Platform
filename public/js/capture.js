...
var videoSelect = document.querySelector('span#videoSource');
...
function takePhoto() {
  imageCapture.takePhoto().then(function(blob) {
    ...
    document.querySelector("img#takePhoto").src = URL.createObjectURL(blob);
    //const file = new File([blob], 'noname', { type: blob.type })
    //document.querySelector("img#takePhoto").value = blob;
   // blob.text().then(text => console.log(text));
    var reader = new FileReader();
    reader.readAsDataURL(blob);
    reader.onloadend = function() {
        var base64data = reader.result;
        document.querySelector("input#image").value = base64data;
